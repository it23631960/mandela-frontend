...
const EmployeePOS = () => {
  ...
  useEffect(() => {
    axios
      .get(import.meta.env.VITE_API_BASE_URL + "/customers")
  ...
  const fetchCustomerByPhone = (phone) => {
    axios
      .get(`${import.meta.env.VITE_API_BASE_URL}/customers`)
      .then((response) => {
        const matchedCustomer = response.data.find(
  ...
  useEffect(() => {
    if (redeemPoints && selectedCustomer) {
      ...
      setDiscount(
        checkBirthdayDiscount(selectedCustomer) +
          (pointsToRedeem / subTotal) * 100
      );
  ...
  const handleCharge = async () => {
    ...
    try {
      ...
      const finalPoints =
        selectedCustomer.loyaltyPoints + pointsToEarn - pointsToRedeem;
      ...
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/customers/${selectedCustomer.id}`,
        {
          ...selectedCustomer,
      ...
      toast.success(
        `Order #${
          orderRes.data.transactionId
        } saved! Total Rs.${orderRes.data.total.toFixed(2)}`
      );
    } catch (error) {
  ...
  const addCustomer = () => {
    axios
      .post(import.meta.env.VITE_API_BASE_URL + "/customers", newCustomer)
      .then((response) => {
  ...
  const handleSendEmail = async () => {
    try {
      await axios.post(
        ...
        }/orders/${transactionId}/send-email`
      );
        ...
        <div className="flex items-left">
          ...
          <div className="flex flex-col ml-2">
            ...
            <button
              ...
              className="text-red-500 text-sm text-left ml-2 cursor-pointer "
            >
              Logout
            </button>
          </div>
          ...
          <div className="w-3/5 p-4 bg-gray-200">
            ...
            <div className="grid grid-cols-5 gap-4">
              {products
                ...
                .filter(
                  (product) =>
                    filter === "View ALL" || product.category === filter
                )
                .slice(0, 25)
                .map((product) => (
                  <div
