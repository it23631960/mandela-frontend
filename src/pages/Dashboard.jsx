import {ComposedChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, Bar, CartesianGrid, Legend, PieChart, Pie, Cell} from "recharts";
...
const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884d8'];
  ...
  useEffect(() => {
    ...
    axios.get("${import.meta.env.VITE_API_BASE_URL}/products")
      .then(response => {
        ...
        const categoryArray = Object.entries(categoryQuantities).map(([name, value]) => ({
          name,
          value
        }));
    ...
    axios.get("${import.meta.env.VITE_API_BASE_URL}/orders")
      .then(response => {
        ...
        const monthlyTotal = monthlyOrders.reduce((total, order) => 
          total + (order.total || 0), 0);
        
        const average = monthlyOrders.length > 0 
        ...
        const weeklyStats = last7Days.map(date => {
          const dayOrders = response.data.filter(order => 
  ...
  axios.get("${import.meta.env.VITE_API_BASE_URL}/customers/new-loyalty-customers")
  .then(response => {
  ...
  .catch(error => {
    ...
    return (
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Sale", value: `Rs. ${monthlyIncome.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` },
          { label: "Average Order Value", value: `Rs. ${averageOrderValue.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` },
          { label: "New Orders", value: monthlyOrderCount },
          { label: "New Loyalty Customers", value: newLoyaltyCustomers },
        ].map((item, index) => (
          <div key={index} className="bg-gradient-to-r from-gray-900 to-black p-4 rounded-2xl shadow-md border-2 border-gray-800 flex justify-between items-center transform transition-transform duration-200 hover:-translate-y-1"> 
        ...
        <div className="bg-white text-black p-5 rounded-2xl shadow-md pt-0 pr-0 pl-0 border-2 border-gray-200 flex flex-col items-center col-span-3">
          ...
          <div className="w-full flex justify-center items-center">
            <ResponsiveContainer width="90%" height={200}>
              <ComposedChart data={selectedView === 'weekly' ? weeklyData : monthlyData}>
                <XAxis dataKey={selectedView === 'weekly' ? 'day' : 'month'} stroke="#ccc" />
